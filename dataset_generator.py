    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None, **kwargs):
        ...
        if self.capturer.is_capturing and self.capturer.step <= 1:
            ...
            with torch.no_grad():
                ...
                # Normalizzazione (Specifica di FLUX)
                query = attn.norm_q(query)
                key = attn.norm_k(key)
                encoder_query = attn.norm_added_q(encoder_query)
                encoder_key = attn.norm_added_k(encoder_key)
                
                # 2. Joint Attention (Unione Testo + Immagine)
                query = torch.cat([encoder_query, query], dim=1)
                key = torch.cat([encoder_key, key], dim=1)
                
                query = query.view(batch_size, -1, attn.heads, query.shape[-1] // attn.heads).transpose(1, 2)
                key = key.view(batch_size, -1, attn.heads, key.shape[-1] // attn.heads).transpose(1, 2)
                
                # 3. Calcolo Pesi Attenzione
                attn_weights = torch.matmul(query, key.transpose(-1, -2)) * (query.shape[-1] ** -0.5)
                attn_weights = torch.softmax(attn_weights, dim=-1)
                
                # 4. Estrazione Cross-Attention (Immagine -> Testo)
...
class FluxDataCapturer:
    ...
    def _transformer_hook(self, module, input, output):
        if self.is_capturing and self.step == 0:
            self.x0 = input[0].detach().clone().cpu()
            self.v0 = output[0].detach().clone().cpu()
            logger.info("--> Hook Base: x0 e v0 estratti con successo.")
        self.step += 1

    def attach(self):
        # 1. Attacca hook generale
        self.handles.append(self.transformer.register_forward_hook(self._transformer_hook))
