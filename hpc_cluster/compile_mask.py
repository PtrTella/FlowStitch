        ...
        for j in range(i + 1, len(tokens) + 1):
            ...
            if target_word in chunk:
                ...
                if target_word not in chunk_minus_first and target_word not in chunk_minus_last:
                    logger.info(f"Ricostruzione semantica trovata: '{chunk}' -> Indici: {list(range(i, j))}")
                    return list(range(i, j))
    return []

def compile_semantic_mask(db_path: str, word_to_isolate: str, manual_tokens: list = None):
    ...
    if manual_tokens is not None:
        logger.warning(f"OVERRIDE MANUALE: Utilizzo gli indici token forzati: {manual_tokens}")
        token_indices = manual_tokens
    else:
        token_indices = find_token_indices(tokenizer, target_prompt, word_to_isolate)
        
    if not token_indices:
        raise ValueError(f"CRITICO: Parola '{word_to_isolate}' non trovata nel prompt '{target_prompt}'. Prova con manual_tokens.")
    ...
    logger.info(f"Shape finale: {A_target.shape} | Range: [{A_target.min():.2f}, {A_target.max():.2f}]")
